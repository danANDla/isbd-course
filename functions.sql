...
select * from add_item_to_order(1,2,1);

create or replace function add_item_to_order(p_id bigint, c_id bigint, u_id bigint) returns integer as $$
declare
    mview record;
    remain real;
    new_quantity real;
...
create or replace function add_product_to_purchase(p_id bigint, prod_id bigint, quantity real) returns integer as $$
declare
    remain real;
    new_quantity real;
begin
